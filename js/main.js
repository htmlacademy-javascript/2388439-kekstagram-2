...
const descriptions = [
  ...
  "Тёплый пляж, прозрачная голубая вода. Над морем летит двухмоторный самолёт. В воде отдыхают люди. Молодая девушка в купальнике с поднятыми руками стоит спиной, приветствуя самолёт.",
  ...
  "Женские ноги в белых тапочках с яркой светодиодной подсветкой. Тапочки освещают путь по темноте, предотвращая столкновения с мебелью.",
...
const descriptionPublishedPhoto = function (){
  ...
  const randomId = getRandomInteger(0, identifier.length - 1);
  const randomDescription = getRandomInteger(0, identifier.length - 1);
